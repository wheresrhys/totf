...
-- core_stats carrying the wing/weight summary statistics (max/avg/min/median
-- for both), split into its own RPC rather than left folded into core_stats'
-- already-large single query — both for query-plan simplicity and to leave
-- core_stats' existing columns/performance untouched (those columns are
-- removed from core_stats in a later ticket, once consumers migrate). Shares
-- its input signature with core_stats / population_stats (species_name_filter,
...
-- columns and their rounding mirror core_stats.sql's final SELECT exactly
-- (ROUND(..., 1) for avg/median weight, ROUND(..., 0) for median wing); the two
-- grouping/identity columns (species_name, time_period) mirror core_stats_result.
--
