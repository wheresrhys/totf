-- Public, group-gated wrapper around abundance_stats (#828, step 1 of the
...
-- abundance_stats/abundance_stats_result. public_aggregate_stats is left
...
-- It returns exactly what abundance_stats returns for the same params, or
-- nothing at all when the group has not opted in / does not exist / no group
-- is given. Execute is granted to the anon role (mirroring abundance_stats) so
...
-- inaccessible.
-- Shares abundance_stats's input signature (forwarded verbatim below) and its
-- return shape via the public.abundance_stats_result composite type, so the
-- two can never drift.
CREATE FUNCTION public.public_abundance_stats (
...
) RETURNS SETOF public.abundance_stats_result LANGUAGE plpgsql SECURITY DEFINER
SET
