...
CREATE POLICY group_birds_access ON public."Birds"
  FOR SELECT
  USING
    ((((((auth.jwt() -> 'app_metadata'::text) ->> 'ringing_group_id'::text))::bigint = ANY (ringing_group_ids)) OR ((ringing_group_ids = '{}'::bigint[]) AND ((((auth.jwt() ->
    'app_metadata'::text) ->> 'ringing_group_id'::text))::bigint IS NOT NULL))));

CREATE POLICY group_birds_insert ON public."Birds"
  FOR INSERT
  WITH CHECK (((((auth.jwt() -> 'app_metadata'::text) ->> 'ringing_group_id'::text))::bigint IS NOT NULL));

CREATE POLICY group_birds_update ON public."Birds"
  FOR UPDATE
  USING (((((auth.jwt() -> 'app_metadata'::text) ->> 'ringing_group_id'::text))::bigint IS NOT NULL))
  WITH CHECK (((((auth.jwt() -> 'app_metadata'::text) ->> 'ringing_group_id'::text))::bigint IS NOT NULL));
