...
CREATE TRIGGER trigger_add_bird_ringing_group_id
...
CREATE TRIGGER trigger_encounters_refresh_bird_proven_age
  AFTER INSERT OR DELETE OR UPDATE ON public."Encounters"
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_encounters_refresh_bird_proven_age();

CREATE TRIGGER trigger_remove_bird_ringing_group_id
  BEFORE DELETE ON public."Encounters"
  FOR EACH ROW
  EXECUTE FUNCTION public.remove_bird_ringing_group_id();

CREATE TRIGGER trigger_set_encounter_generated_fields
  BEFORE INSERT OR UPDATE OF session_id, capture_time ON public."Encounters"
  FOR EACH ROW
  EXECUTE FUNCTION public.set_encounter_generated_fields();

CREATE TRIGGER trigger_update_bird_ringing_group_id
  AFTER UPDATE OF ringing_group_id ON public."Encounters"
  FOR EACH ROW
  EXECUTE FUNCTION public.update_bird_ringing_group_id();

CREATE POLICY group_encounters_access ON public."Encounters"
  FOR SELECT
  USING ((ringing_group_id = (((auth.jwt() -> 'app_metadata'::text) ->> 'ringing_group_id'::text))::bigint));

CREATE POLICY group_encounters_insert ON public."Encounters"
  FOR INSERT
