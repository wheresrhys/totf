...
CREATE FUNCTION public.trg_refresh_ring_sequence_bounds()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
ALTER TABLE public."Birds" ADD COLUMN ring_sequence_id bigint;
ALTER TABLE public."Birds" ADD CONSTRAINT birds_ring_sequence_id_fkey FOREIGN KEY (ring_sequence_id) REFERENCES public."RingSequences"(id);
CREATE INDEX idx_birds_ring_sequence_id ON public."Birds" (ring_sequence_id);
CREATE TRIGGER trigger_trg_refresh_ring_sequence_bounds AFTER INSERT OR UPDATE OF ring_sequence_id ON public."Birds" FOR EACH ROW EXECUTE FUNCTION public.trg_refresh_ring_sequence_bounds();
