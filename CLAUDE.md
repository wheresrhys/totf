...
`core_stats/robin-alpha.species-page-composite.json`). A fixture built from more than one RPC/table
...
action(s) for exactly this reason. Regenerate every fixture here with
...
surfaces in the fixture rather than drifting silently. The same applies to the merged shapes —
build a `CoreStatsWithBiometrics` row with `mergeBiometricsFields` and a `SpeciesStatsRow` with
`mergeSpeciesBiometrics` over the two source fixtures, exactly as the production read paths do,
rather than reading wing/weight columns off a `core_stats` fixture (`core_stats` stopped carrying
its own copies at #827).
...
check the two `synthetic/` fixtures, which no generator produces (they can only be edited by hand).
