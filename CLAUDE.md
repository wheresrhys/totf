...
- **Models** (`app/models/`) hold domain types and pure transformation logic — no I/O.
  - Session highlights are plain-data objects (`app/models/session-highlights.ts`): `derive*` functions produce a `SessionHighlight[]` pool, which `fetchSessionHighlights` passes through the highlight machine in `app/models/highlight-refinement-machine/` — `index.ts` reduces the pool through an ordered `RULES` list (each rule fed the previous rule's output; combining precedes the final ordering rule, which sorts purely by each highlight's `sortValue` — a two-part `{ family, orderWithinFamily }` key stamped at creation: family gives the fixed editorial priority (the ordered `HIGHLIGHT_FAMILIES` list), `orderWithinFamily` ranks highlights sharing a family and is bumped above its parts when highlights are combined), one rule per file (`remove-*`, `combine-*`, `order-*`). The plain data serialises across the server-action boundary; the client renders each via `renderHighlight` in `app/components/session-highlight-renderers.tsx` (a `type`-keyed renderer map, mapped-typed so every variant must have a renderer). Editorial refinements belong in a machine rule (removal/ordering/combining) or a renderer (rewording); add a rule by writing a file and slotting it into `RULES`.
