...
`find_discrepencies/`, `notable_retraps/`, `top_metrics_by_period/`, `ring_sequence_controls/`), or
...
`core_stats/robin-alpha.monthly-history.json`). A fixture built from more than one RPC/table
call in a single generation block (e.g. a species-page composite of `top_metrics_by_period` +
`core_stats` + a `Birds` table read) is filed under whichever source carries the most
drift-risk for that fixture's purpose — see the comment above the relevant block in
`scripts/generate-snapshots.ts`, which documents every fixture's actual RPC/table and consuming
action(s) for exactly this reason. Not every fixture here is generated by that script — a handful
(`ring_sequence_controls/`, `tables/Encounters/`, `tables/Species/`, and the
...
**Fixture drift is caught by a pre-push, diff-gated check — but only for the 19 generated
...
- **What it covers.** Exactly the 19 fixtures `scripts/generate-snapshots.ts` writes
...
- **What it doesn't.** The 8 hand-maintained fixtures (`UNGENERATED_SNAPSHOT_FIXTURES` — the
  ...
  `tables/Encounters/`, `tables/Species/`). A second coverage test pins the on-disk file list to
  generated + ungenerated, so the gap stays visible rather than implied-fixed. Bringing them under
  the generator is [#894](https://github.com/wheresrhys/totf/issues/894).
...
fixtures in the same PR, and hand-edit the eight ungenerated ones. **The generator must produce
what the consuming action produces, not what the raw RPC returns** — e.g. `fetchSpeciesData`,
`getSpeciesStatsHistory` and the species page's `speciesStats` all merge `biometrics_stats`'
wing/weight columns onto their `core_stats` rows, so the generator merges them too. A generator
that skips a merge the action performs makes the fixture lie about the shape components receive,
and the freshness check will happily bless it.
