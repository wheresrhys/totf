import type {
  HighlightScope, HighlightRanking, TemporalUnit, CombinedHighlight
} from '../../types';
...
function getBaseScope (withSpecies: boolean, temporalUnit: TemporalUnit): HighlightScope {
  return withSpecies ? { temporalUnit , species: 'egg'}: {temporalUnit}
}

function getCombinedHighlight(scopes:{
  scope: HighlightScope;
  ranking: Partial<HighlightRanking>;
  }[], withSpecies: boolean): Partial<CombinedHighlight> {
  return {
    ...
    species: withSpecies ? 'egg' : undefined,
    bestPosition: Math.max(...scopes.map(scope => scope.ranking.position as number)),
    scopes: scopes as {
      scope: HighlightScope;
...
export function getFixtures (withSpecies: boolean): Record<string, Partial<CombinedHighlight>> {
  return {
